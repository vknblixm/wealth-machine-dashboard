...
export function generateRevenueData(): Revenue {
  ...
  const timeMultiplier = 1 + (Date.now() % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60 * 24) * 0.5;
  ...
  const streams: RevenueStream[] = [
    ...
    },
    {
      ...
      type: 'coaching',
      hourlyTarget: 1800,
      hourlyActual: Math.floor(1800 * (0.85 + Math.random() * 0.3) * timeMultiplier),
      dailyTarget: 22000,
      dailyActual: Math.floor(22000 * (0.75 + Math.random() * 0.45) * timeMultiplier),
      monthlyProjection: 550000,
      conversionRate: 32 + Math.random() * 14,
      avgCustomerValue: 3500,
      status: 'scaling',
      margin: 78,
      growthRate: 16 + Math.random() * 9,
    },
  ];
